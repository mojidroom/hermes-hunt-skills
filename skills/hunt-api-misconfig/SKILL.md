...
## MDPsec Verified Patterns (real API-misconfig reports)

Real-world primitives from mdpsec.com reports:

1. **Public OAuth client for internal tool** — back-office catalog tool registered as PUBLIC client → free account creds + public client id (no secret) → internal tool identity + broad catalog scope; junk secret → error, OMITTING secret → privileged token; write to live billing catalog with forgeable audit field (85).
2. **OAuth audience confusion** — Sysadmin API accepts tenant-app token despite `aud = TENANT_APP_CLIENT` (not SYSADMIN), obtained via username/password (no `identities` claim), user type Tenant; one route missing tenant-scope check among correctly-gated siblings → cross-tenant data + internal staff details (76).
3. **Anonymous endpoint returns data with empty token** — `POST /pos-self/data/1` JSON-RPC `{"access_token":""}` → full response incl. self_ordering_url with valid 16-char hex bearer token in query string; should be 401/403 (82).
4. **Auth on session but not RBAC** — fleet endpoints check login token but not fleet-management permission; rider token → full fleet data 200 (96).
5. **Enum echo leaks internal roles** — invalid business_type value returns full enum in validation error (31).
6. **OpenAPI spec exposure** — `/api/api-docs` full spec (170 endpoints) reachable unauthenticated with no security block; embedded internal emails/phones prove production (11).
