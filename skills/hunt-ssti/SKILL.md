...
- **`triage-validation`** — Apply the Pre-Severity Gate before claiming Critical RCE. A `{{7*7}} → 49` reflection inside a sandboxed engine (e.g., Twig sandbox mode, Jinja2 SandboxedEnvironment with no escape) is Medium SSTI, not Critical RCE. Prove `id`/OOB DNS callback with a unique marker before writing the report.## webhacklist 2024-2026 updates

- **Blind SSTI — 2025 top technique (2025 #1, Research_Successful_Errors)** — No visible reflection. Oracles: time-delay (`{% if %}` + `sleep`), DNS/OOB (`{{...}}` + `http://collab`), error differential. Test every injectable field even if output is never echoed.
- **SpEL Casting and Evil Beans** — 2022. Expression injection in Spring `#{...}`/`${...}` with type casting to gadget invocation.
- **Transformers: Dark Side of the Type (Conversion Layer)** — 2026. Weaponize a framework type-conversion layer (auto-converting binder/`@Converter`) that deserializes a string into an object → gadget.
