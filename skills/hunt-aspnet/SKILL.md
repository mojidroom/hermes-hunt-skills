...
## 🏆 HX007 — Exploring IIS in Bug Bounties (Conference)

Full ref: [`references/hx007-iis-bug-bounty-techniques.md`](references/hx007-iis-bug-bounty-techniques.md)

### ASP.NET Extensions to Fuzz (Beyond .aspx/.asp)
```bash
ffuf -u "https://target.com/FUZZ" \
  -w directory-list-lowercase-2.3-medium.txt \
  -e .xml,.txt,.zip,.7z,.dll,.ashx,.asmx,.svc,.html,.htm,.js,.json
```

| Extension | Real Examples |
|-----------|--------------|
| `.xml` | Database configs |
| `.zip` | `api.zip`, `wwwroot.zip` |
| `.7z` | `bin.7z` |
| `.dll` | `web.dll` |
| `.ashx` | `UploadHandler.ashx` |
| `.asmx` | `File_Manager.asmx` |
| `.svc` | `Service1.svc` |
| `.json` | `appsettings.json` |

### ASP.NET Cookieless Sessions — WAF Bypass
```
http://target.com/(S(sessionid))/default.aspx
```
Session ID in URL bypasses WAFs that only check Cookie headers.

### WAF Bypass for ASP.NET
- **AWS IP Rotation** — Fast, expensive
- **Bright Data Rotating Proxies** — Residential IPs
- **ASP.NET Cookieless Sessions** — URL-based session bypass
- **Protocol downgrade** — HTTP/2 → HTTP/1.1 → HTTP/1.0
