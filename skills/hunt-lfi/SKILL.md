...
## MDPsec Verified Patterns (2 real LFI reports)

Real-world primitives from mdpsec.com reports:

1. **ImageMagick `text:` coder LFI via SVG upload** — upload SVG with `<image xlink:href="text:/etc//passwd">`; the file-reading coder paints target file bytes into the stored photo; served on public image host with no auth → rendered file contents fetchable by anyone (54). Signature filter blocks obvious paths but **doubled-slash normalization** `text:/etc//passwd` bypasses. Confirmed reads: /etc/passwd, hostname, dependency manifest, DB config, secret-management initializers, hardcoded production third-party key+secret. Same sink fetches URLs server-side and follows redirects → blind SSRF + OOB callbacks from production IPs.
2. **Connector TLS key via trust-certificate path** — DB-sink connector config accepts `db.trust.certificate.path=<path>`; worker opens path, cert parser rejects bytes, raw file contents copied verbatim into connector task trace field; trace readable by any `read_only` project member via status/logs endpoints (105). Bypasses systemd `InaccessiblePaths=` hardening — cert path reads anything not on the list. Also reads /etc/passwd, /proc/self/environ, /proc/self/cmdline, /proc/self/mountinfo, /etc/hosts, /proc/version. Connector self-deletes after read (stealthy).
3. **Detection**: poll status endpoint until trace contains `BEGIN PRIVATE KEY`; cross-role proof = read-only user gets 403 on create but reads admin-created connector's key byte-for-byte (SHA-256 match).
