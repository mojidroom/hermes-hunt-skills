...
## MDPsec Verified Patterns (2 real Supabase reports)

Real-world primitives from mdpsec.com reports:

1. **RLS intent proof** — anon key (public JWT in frontend JS) grants anonymous role; **INSERT blocked but SELECT/UPDATE/DELETE open** on most tables → developers intended to restrict anonymous access, policies were simply incomplete. Test the FULL permission matrix across ALL tables (106).
2. **GraphQL over exposed tables** — campaign stats: 28,753 wallet addresses + trading volumes; airdrop: 16,089 more; UPDATE on campaign stats/airdrop/user profiles → leaderboard/prize manipulation; DELETE on 10+ tables → mass deletion; anonymous RPC execution (leaderboard reset) (106).
3. **Anon-role RPC without authz checks** — `POST /rest/v1/rpc/get_invoices_for_email_action` with `{}` + anon apikey → bulk dump of 50 cross-tenant invoice records (business name, amounts, hosted invoice URL + PDF accessible without further auth) (107).
4. **Config JS file leaks project URL + anon key** — web app loads Supabase config from public JS file; PostgREST verbose error hints suggest correct function name on wrong-name calls → schema enumeration (107).
