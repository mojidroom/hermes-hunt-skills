...
**5b. WordPress REST API multi-endpoint CORS validation:**
```html
<!doctype html><body><pre id="out"></pre>
<script>
...
- **Endpoint-specific policy:** CORS headers may be absent on `/` but present
  on API routes. Test the bounded authenticated endpoints identified during
  application mapping, not only the root.
...
## MDPsec Verified Patterns (real CORS reports)

Real-world primitives from mdpsec.com reports:

1. **ACAO reflection + credentials on subdomains** — data API reflects ANY subdomain of parent domain into `Access-Control-Allow-Origin` with `allow-credentials: true`; session cookie scoped to parent domain with cross-site setting → content-origin page (attacker-hosted HTML on a first-party subdomain via upload) makes authenticated requests to the data API and reads responses: registered email, legal name, KYC status, wallet address, sub-account topology, referral graph, session IPs/devices/UAs (39).
2. **Chain: presigned upload → Content-Type XSS → CORS exfil** — upload HTML payload to image key with `text/html` type on first-party origin (no nosniff/CSP) → page executes same-origin → authenticated requests to data API via reflected ACAO + credentials → victim's own data exfil'd when a separate logged-in user opens the stored payload (39).
3. **Wildcard CORS on PII endpoint** — order-details endpoint has wildcard CORS + no credentials → any attacker web page can read responses directly from visitors' browsers → distributed harvesting defeats per-address rate limiting (71).
4. **CORS allowlist bound to main app origin** — GraphQL endpoint CORS allowlist bound to main app origin → calls work from browser console on the login page (77).
