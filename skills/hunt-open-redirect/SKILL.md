...
## MDPsec Verified Patterns (real open-redirect reports)

Real-world primitives from mdpsec.com reports:

1. **Open redirect as deep-link WebView pivot** — carrier logout endpoint 302s to whatever `TARGET` query param says with no auth/allowlist; deep-link handler validated only incoming URL's host == carrier domain → pointing it at carrier's own open redirect passed the check → WebView loads attacker phishing page inside app chrome → creds captured → redirect back to real homepage (10). Chain: verified-domain link → app opens via App Links → WebView loads carrier logout URL → 302 → attacker page.
2. **Open redirect chaining into SSRF** — 302 redirect service flips POST→GET against IMDS: external redirector issues 302 → server follows → POST becomes GET → metadata reflected (104).
3. **Branch.io open redirect chain** — deep-link-service key lets attacker mint links under verified domain whose payload points at open redirect on same domain → passes host check → redirects to attacker (10).
4. **Redirect-following SSRF expansion** — SSRF sink follows cross-host redirects → 302 to different domain with misleading content-type → reflected media.type matches final destination (75).
