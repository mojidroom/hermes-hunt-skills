...
## MDPsec Verified Patterns (real Next.js reports)

Real-world primitives from mdpsec.com reports:

1. **`NEXT_PUBLIC_*` env exposure in client bundle** — Next.js app exposes env values prefixed for client use; one is a production blockchain data provider API key in plaintext inside the public `_app` JS chunk served to every visitor. Find: fetch homepage, grep for `_app` chunk reference → bundle URL; curl bundle, grep for provider variable name → full RPC URL with key as query string param (74).
2. **Webhook CRUD by query-string key** — provider's webhook endpoint accepts key as query-string credential → unauthenticated POST creates webhook (201, returns id); DELETE → 200; same primitive deletes the platform's operational webhooks (74).
3. **Multiple keys in same bundle** — systemic secret-in-bundle pattern; grep built JS artifacts for key patterns (not just source) (74, 95).
4. **RPC gateway key in bundle** — key lives in URL path of RPC endpoints embedded in public JS bundle → `debug_traceBlockByNumber`, `eth_getBlockByNumber("pending", true)`, `eth_sendRawTransaction` (relay) reachable from browser context; valid token → 200, randomized → 404/401 (95).
