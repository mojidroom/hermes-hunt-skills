...
## MDPsec Verified Patterns (4 real OAuth/identity reports)

Real-world primitives from mdpsec.com reports:

1. **Device-link state hijack** — token bound to device-household identifier that is FULLY attacker-chosen and reflected verbatim into sign-in `state`; attacker mints link code bound to their household, polls for victim's token after authorization; sign-in client accepts attacker-supplied scope upgrade read-only → read-write; email-change endpoint needs no password/confirmation → permanent ATO (28).
2. **Audience confusion** — tenant Admin App token accepted by staff-only Sysadmin API despite three disqualifying signals: `aud = TENANT_APP_CLIENT` (not SYSADMIN), obtained via username/password (no `identities` claim), user type Tenant. One route missing tenant-scope check among correctly-gated siblings → cross-tenant job data + internal staff details (76). Test: take your low-trust app token, hit admin/staff origins, check for missing audience validation.
3. **Internal tool as public OAuth client** — back-office catalog tool registered as PUBLIC client → anyone with valid account creds (no client secret) gets internal tool's identity + broad catalog scope; junk secret → error, OMITTING secret entirely → privileged token (85). Test: try token endpoint without client_secret.
4. **Wildcard postMessage login code** — `opener.postMessage(loginSuccessMessage, '*')` on real login origin `?flow=popup` → attacker opener receives OAuth code; 100-hour bearer; silent PATCH email no re-auth (92).
...
- **Cookie tossing to hijack OAuth flows** (2024 #10, 'Hijacking OAuth flows via Cookie Tossing') — Seed a separate attacker-managed cookie with the same `name=value` on the auth domain (subdomain cookie toss + `__Host-`/`domain` scoping), then force the victim's OAuth cookie set to collide so a session/`nonce`/`pkce_code_verifier` lands under attacker control. Test: find an IdP/auth endpoint that trusts a cookie-set you can toss from another subdomain; write your value first, then trigger the victim's authorize callback and observe which cookie wins.
- **Authorization-code injection** (2025, 'Attacks via a New OAuth flow, Authorization Code Injection') — Inject an attacker-obtained authorization code (unsolicited `code`) into a victim's `/callback` when the client fails to bind `code` to the original `state`/PKCE session. Test: complete your own authz flow, capture the code, then feed it into a fresh victim browser session (`/callback?code=<attacker_code>`) and check whether it links the victim's account instead of erroring.
- **OAuth dirty dancing / token replay across clients** (2022 #1, 'Account hijacking using dirty dancing in sign-in OAuth-flows') — Replay an auth code/REST token to an unrelated OAuth client that shares the same token-issuer or callback handler (id-token not bound to `audience`/`client_id`). Test: take a token issued for client A and replay it to client B's `/callback` (or callback-parameter swap across `login_hint`/`redirect_uri`) and inspect whether B accepts it for the victim identity.
- **Non-happy-path token replay** (2024 #8, 'OAuth Non-Happy Path to ATO') — Trigger the error/denied branch (`?error=access_denied`, revoked grant, repeated code reuse) to obtain a code/token that the happy path would not expose, then replay it across clients/sessions. Test: request the authz endpoint with `error`-forcing or double-exchanged code and check for under-validated tokens still redeemable for a victim account.
- **Cross-app universal OAuth attacks** (2025, 'Universal Cross-app Attacks') — In integration platforms (workflow/automation hubs where many third-party OAuth apps share one authorize callback surface), attack the shared token store via universal redirect_uri, cross-client audience confusion, or app-context delegation. Test: enumerate all integrations sharing one callback root, cache-miss a universal `redirect_uri` accepted by multiple apps, and replay one app's token into another integration's context.