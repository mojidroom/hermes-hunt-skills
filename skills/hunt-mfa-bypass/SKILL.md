...
## MDPsec Verified Patterns (real MFA/OTP reports)

Real-world primitives from mdpsec.com reports:

1. **Concurrent OTPs not invalidated** — unauthenticated email-OTP endpoint accepted any email; MULTIPLE unredeemed codes valid simultaneously for 30 min, NOT invalidated on new send → attacker triggers OTP send for victim email, victim types code into attacker's fake form, attacker logs in from separate device with captured code while other codes still valid (34).
2. **Blank code accepted (async write TOCTOU)** — login creates verification record synchronously but writes the code asynchronously → 0–1s window where record exists with EMPTY code field; verification endpoint has no null check → blank code accepted as valid → authenticated session → wallet drain (65).
3. **2FA enforcement asymmetry across chains** — withdrawal-address creation accepted empty OTP field for non-memo chains (BTC, ETH, USDT ERC20/TRC20, SOL) while only memo-required chains (XRP, XLM) enforced OTP (40).
4. **OTP-adjacent flows without step-up** — order placement rejected only for insufficient balance (no OTP); spot↔perpetual transfers no 2FA; API-key minting flows with no 2FA (40).
