...
- **Python class pollution in-the-wild** (2026, 'Get Set, Exploit! Python Class Pollution In-the-Wild') — Recursive merge functions in Python allow polluting class attributes via `__class__`/`__init__`/`__globals__` chains, turning an object merge into arbitrary attribute/global overwrite (and ergo RCE sinks). Test: send `{"__class__": {"__init__": {"__globals__": {...}}}}` through a JSON-deep-merge endpoint and verify a writable class/global attribute changes (e.g. an auth flag or logger handler).
- **Prototype pollution in Python** (2023) — Python lacks JS `__proto__`, but named merges (`update`, `|=`, recursive `dict` merge, `copy.deepcopy` with `__setattr__`) propagate attacker keys into class dicts/`__globals__`. Test: inject `__class__`/`__init__`/`__globals__` keys into a mergeable nested body and check a class attribute or function global was altered without an explicit setter.
- **Server-side PP black-box detection without DoS** (2023) — Detect server-side prototype pollution black-box by triggering observable reflection (in an output/mirror field, status code, or response header) using a benign polluted marker instead of crashing the process. Test: pollute a harmless property (`__proto__[p]={{7*7}}` or a marker) and look for the marker echoed in a response that reflects the *original* object, confirming the merge mutated `Object.prototype` without killing the app.
- **Class Pollution in Ruby** (2024, 'Class Pollution in Ruby: A Deep Dive into Exploiting Recursive Merges') — Ruby recursive merges (`deep_merge`, `inject`, `merge!`) let attacker keys hit class methods/constants through symbol/hash key pollution (e.g. `:__send__`, instance variable `@x`) enabling logic bypass/RCE. Test: send Hash keys that merge into a class/module (`{polluted: ...}` nested into the object tree) and observe a class-level ivar/constant change or method dispatch altered.
- **Prototype pollution to RCE gadgets** (2022, 'Silent Spring: Prototype Pollution Leads to RCE in Node.js'; 2024, 'Undefined-oriented Programming' + 'GHunter') — Chain a discovered PP primitive into a real gadget that executes code (e.g. `child_process` `shell`/`env` props, `require`/`module` pollution, `__proto__` on `object` used by a grep/`mri` chain). Test: after confirming pollution, set a known RCE gadget property (`__proto__.shell=/proc/self/exe` + a file-write/setuid sink, or `NODE_OPTIONS`/require gadget) and trigger the consuming sink to confirm command execution via `id`/`whoami`.
