...
## MDPsec Verified Patterns (real rate-limit reports)

Real-world primitives from mdpsec.com reports:

1. **GraphQL variant escapes WAF-limited REST sibling** — legacy REST redemption protected by Cloudflare rule (~22 requests) but GraphQL endpoint not covered → 23,000 probes in 85s (~270 rps) with zero throttling; valid PINs returned StoreCredit instantly (44).
2. **Predictable token structure + differential response** — gift card PINs 19 digits: fixed prefix + sequential batch ID + fixed segment + sequential counter; invalid → `BAD_REQUEST` (HTTP 200), valid → `StoreCredit` object → differential = oracle; ±200 from seed with 20-50 concurrent workers (44).
3. **Token-validation endpoint as brute-force oracle** — feature-flag service token-validation endpoint returned 1/0 for candidate tokens, no auth, no rate limit (42).
4. **Sequential enumeration without throttle** — 23,000 gift card probes (44); quote-number sweep 215 in ~3 min zero throttling (57, 59); 100 sequential phone numbers at 2 req/s → 21 real records (12); order counter walk (71); purchase-key sweep 15-17/25 used (98).
5. **No lockout on OTP/verify endpoints** — OTP send/verify with no rate limit; multiple concurrent codes valid (34).
6. **Detection** — always compare WAF-limited REST vs unprotected GraphQL/other-transport variants; watch for differential response shapes that act as oracles.
