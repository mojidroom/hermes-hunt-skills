...
## MDPsec Verified Patterns (2 real race-condition reports)

Real-world primitives from mdpsec.com reports:

1. **Magic-link TOCTOU via async code write** — login creates verification record synchronously but writes the actual code asynchronously (background job) → 0–1s window where record exists with EMPTY code field; verification endpoint has no null check → blank code accepted as valid (65). Full chain: trigger login for victim email → immediately submit blank code → authenticated session → fetch encrypted wallet material → decrypt via cloud KMS → ~60s wallet drain. Works for email + Telegram wallets; Telegram user IDs public → 38,000+ enumerable targets.
2. **Refund race with HTTP-status oracle** — refund flow: app server checks "already refunded?" → forwards credit to downstream payment server → credits gift card → marks refunded; steps 2 and 5 NOT atomic. Fire 20 concurrent requests with unique request IDs: `200 + booking data` (processed), `500 "RefundAlreadyInProgress"` (credit committed — each one is a credit!), `400 "already refunded"` (lock took effect) (87). $5 ticket → +$90-100 credit per cycle (18-20x multiplier).
3. **Detection via error-code taxonomy** — the 200/500/400 split is the oracle proving which requests passed the check before the lock; HTTP 500 = reached payment server = money moved.
...
## a13h1 Medium Writeups — Race Conditions (4 articles, ~$1,000)
- **Race Condition Down a SaaS Global Platform** — `POST /api/v1/jobs` concurrent duplicate submission on SaaS platform
- **$500 Bypassing Business Logic via Race Condition** — Race on business logic flow to bypass restrictions
- **$500 MFA Bypass by Race Condition** — Race condition to bypass MFA verification window
- **Race Condition Exploit: Millionaire in Minutes** — Critical race on financial/digital platform



## Types of Ephemeral Vulns

### 1. Race Conditions (TOCTOU)
```bash
# Check-then-act pattern
# 1. Server checks balance → yes, enough
# 2. Send 20 concurrent requests BETWEEN check and deduct
# 3. All 20 pass → balance goes negative!

seq 20 | xargs -P 20 -I {} curl -s -X POST https://target.com/redeem \
  -H "Authorization: Bearer TOKEN" -d 'code=PROMO10' &
wait
```

### 2. Token Timing Attacks
```bash
# OTP valid for 120s → brute-forceable
# Short-lived session token can be reused within window
# JWT with "exp" claim not validated → eternal token

# Test: reuse old token after expiry
curl -sk https://target.com/api/user -H "Authorization: Bearer OLD_TOKEN"
```

### 3. Rate Limit Window Abuse
```bash
# Rate limit resets every 60s
# 1. Send 5 requests → blocked for 60s
# 2. Wait for window to reset
# 3. Send 5 more → another 60s wait
# → Gradual brute force over time
```

### 4. Temporary URL Access
```bash
# Password reset links, file upload URLs, signed URLs
# Test if they work after expiry
# Test if they work for DIFFERENT users
```

### 5. Continuous Recon (from Jujitsu)
```bash
# Ephemeral vulns require continuous monitoring:
# - New subdomains that appear and disappear
# - Temporary debug endpoints left in production
# - Staging instances accessible during work hours
# - Test endpoints that appear after deployment
```



## Key Insight from Hunt Class Jujitsu
> "Security weaknesses that exist for a limited period of time, typically during a short window of opportunity."



## Disclosed Report Citations — a13h1 Medium Writeups

13. **SaaS Global Platform — Race Condition via Job Duplication** ([Medium](https://medium.com/bugbountywriteup/how-a-simple-race-condition-vulnerability-down-a-saas-global-platform-8093528b403f))
    - Subclass: race condition on job creation endpoint (SaaS platform)
    - Endpoint: `POST /api/v1/jobs`
    - Payload: concurrent duplicate job submission requests
    - Root cause: No atomic check on job creation — concurrent requests bypassed duplicate detection
    - Platform: ExampleCenter (SaaS collaboration platform)
    - Year: 2025 — InfoSec Write-ups publication
    - Key takeaway: Race conditions on SaaS workflows (job/task creation) can cause data duplication, financial impact, and workflow corruption. Test every "create" endpoint for concurrent submission behavior.
...
- [ ] Test race conditions (send concurrent requests)
- [ ] Test token reuse after expiry
- [ ] Test rate limit windows
- [ ] Test temporary URLs after intended expiry
- [ ] Monitor continuously for new ephemeral surfaces
- [ ] Test TOCTOU in payment/balance flows
- [ ] Test OTP codes after intended window



## Real Examples
- **$49,000 Instagram vulnerability** (ephemeral race condition)
- **$1,500 Snapchat vulnerability** (ephemeral token reuse)



## References
- Notion: Hunt Class Jujitsu (Ephemeral Vulnerabilities page)
- "Catch Me If You Can" - Shubham Shah & Michael Gianarakis at 44CON 2018

## What are Ephemeral Vulns?
Vulnerabilities that exist only during a specific time window:
- Race conditions (TOCTOU)
- Temporary tokens that expire but can be reused
- Rate limits that reset after a window
