...
## MDPsec Verified Patterns (real misc reports)

Real-world primitives from mdpsec.com reports:

1. **Forwarded-header trust boundary (CWE-348)** — compliance endpoints derive geographic state from attacker-controlled `X-Forwarded-For` instead of network-layer source IP; origin reflects supplied value back as "client IP"; LEFT-MOST value wins → attacker value always wins even behind proxy that appends real client IP (43).
2. **Vite dev server in production** — `/@fs/` serves allowlist-root files with no auth; 403 discloses allowlist root; single GET downloads SQLite DB + source + .git with GitHub PAT (118).
3. **Public exec dashboard** — `GET /internal/dashboard` 200 unauth; `dt_produced_next` timestamp proves live; $30M reserves + operational telemetry (112).
4. **Session replay SDK backdoor** — exported deep link activity writes attacker key-value pairs into SharedPreferences → override recording upload endpoint + disable input masking; persists across restarts (99).
5. **Webhook blind SSRF oracle** — `POST /api/webhook/test` with attacker URL; error message echoes upstream status code → 2xx-vs-non-2xx oracle → internal service mapping (123).
6. **Blind SSRF status-code oracle** — upstream non-2xx → `"Invalid response code from webhook url: 404"`; 2xx wrong format → `"Webhook challenge failed"` (123).
7. **Feature-flag admin endpoints public** — 116 feature flags, token inventory, segment leaks 23 privileged ID hashes, token-validation brute-force oracle (42).
8. **Public OAuth client privesc** — internal tool registered as public client; OMITTING secret → privileged token (85).
9. **Unauth warranty claim → real CRM case** — client-side-only screening; `loginStatus='Anonymous'`; real case created in production CRM (117).
10. **Zero-click screen recording hijack** — session replay SDK deep link backdoor; `configure` param overrides upload endpoint + disables masking (99).
11. **Push notification takeover** — unauth user-service device register/delete/re-register; list-users endpoint returns device tokens (114).
