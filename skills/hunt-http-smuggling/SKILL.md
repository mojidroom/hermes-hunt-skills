...
Post-2023 desync evolutions that bypass the RFC 9112-strict 2026 reality check. Test these against HAProxy ≤2.4, older F5/Citrix, custom Go/Python proxies, and any chain where the backend parser is laxer than the front-end. Always confirm with a cross-client side effect, never your own follow-up.

- **TE.0** (Unveiling TE.0, 2024 #3; thousands of Google Cloud sites) — send `Transfer-Encoding: chunked` with only a terminating `0\r\n\r\n` chunk and no declared Content-Length; front-end treats the request as complete, backend waits for the next chunk of the next request. Test: raw-socket send `POST / HTTP/1.1\r\nHost: t\r\nTransfer-Encoding: chunked\r\n\r\n0\r\n\r\nGET /victim HTTP/1.1\r\n...` on one keep-alive connection and check the follow-up lands on the smuggled path.
- **CL.0 for C2** ('The Quiet Side Channel... CL.0 for C2', 2025) — send `Content-Length: 0` on a request that logically should carry a body; backend reads the next pipelined request's bytes as the dangling body and desyncs. Test: `POST / HTTP/1.1` with `Content-Length: 0` plus `Connection: keep-alive`, then immediately a second GET on the same socket; if the backend blocks waiting for body bytes, CL.0 confirmed.
- **CRLF-powered desync** ('CRLF-Powered Desync Attacks: Beheading HTTP Streams', 2026) — inject raw CRLF into chunk framing, chunk-size lines, or header separators to behead the request stream; one parser honors the injected boundary, the other does not. Test: vary chunk-size line terminators and inline CRLF bytes between chunks, replay with Turbo Intruder, watch for a follow-up response being consumed as the smuggled request's reply.
- **Funky chunks / ambiguous chunk line terminators** (2025) — use CR-only (`\r`) or `CRCRLF` instead of the canonical `CRLF` after chunk sizes; permissive backends split on bare CR while RFC-strict front-ends do not. Test: `\x0d`-terminated chunk-size lines in a raw HTTP/1.1 POST, confirm different boundary parsing across the two tiers.
- **Single-packet request tunneling / desync** ('The Single-Packet Shovel: Desync-Powered Request Tunnelling', 2025) — pack two requests into one TCP packet such that front-end and back-end delimit the boundary in different places; enables request tunneling and response-queue confusion without connection reuse. Test: craft one send() combining a CL-delimited POST plus a nested GET, check whether the backend returns the nested response on the same packet's reply.
- **HTTP Trailer parsing discrepancies** ('Trailing Danger', 2025) — smuggle via `Trailer:`-declared headers whose bytes are mis-parsed; some backends consume trailer field lines as part of the following request. Test: send a chunked POST ending with a `Trailer: X` line plus dangling trailer bytes, then a follow-up request, and check for response desync.
- **HTTP/2 CONNECT abuse** ('Playing with HTTP/2 CONNECT', 2025) — abuse `CONNECT` semantics over HTTP/2 to tunnel arbitrary octet streams to the origin, bypassing normal request parsing and enabling covert request smuggling / tunneling. Test: issue an extended CONNECT with a crafted :path/:authority over H2 and inspect whether the origin treats the subsequent stream as a desynced request.

Verify each with the HA Rank timing and cross-client side-effect gates before reporting; TE.0 and CL.0 are the highest-yield of these in 2024-2026 GCP/Cloudflare-fronted stacks.
