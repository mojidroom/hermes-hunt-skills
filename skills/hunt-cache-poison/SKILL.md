...
- **Cache-key injection / smuggling poison through the cache key door** (2026 watchlist, 'Cache Key Injection: Smuggling Poison Through the Door') — Insert ambiguity into the cache key (encoded chars, delimiter collisions, header-derived keys) so attacker-controlled payloads become part of the keyed business-logic inputs and get cached as trusted content. Test: fuzz cache-key components (URL encoding, `?`, `&`, path-delimiter, `Vary` header) with an injected poisoning marker and check whether the poisoned body is served to a clean-key request.
- **Next.js fetch caching + unkeyed inputs chains** (2025 #7, 'Next.js, cache, and chains: the stale elixir') — Next.js static-`fetch`/`cache` auto-caching caches on `URL+method` only, ignoring auth headers/cookies/query params, so an unauthenticated attacker poisons a cached SSR response replayed to authenticated visitors. Test: `fetch(url, {cache:'force-cache'})` endpoint that keys on URL only — cache a request without cookies under a victim URL, then replay with valid session cookies and confirm the stale unauthenticated body is served.
- **Wildcard web cache deception** (2024 #9, 'ChatGPT Account Takeover - Wildcard Web Cache Deception') — Cache a wildcard path segment (`/sessions/{id}.css`, `/.next` assets, arbitrary extension) that mirrors the underlying page, storing authenticated responses as public static content. Test: request an authenticated URL with a crafted wildcard suffix (`/account/{wildcard}.css` variant) without auth and read the cached victim-private body; verify cache path-extension bypass.
- **Unkeyed query/port/scheme cache poisoning** (2024, 'Gotta cache em all: bending the rules of web cache exploitation') — Poisons cache keys that normalize/ignore scheme, port, or query parameters so attacker-controlled unkeyed inputs are stored and served to all users of the key. Test: vary query string, `Host`/`port`, or scheme (`http` vs `https`) on the cache key while injecting a poisoned body and confirm the cache serves it back on an unrelated clean request.